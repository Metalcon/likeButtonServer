...
public class PersistentLikeHistory {
     ...
     */
    private static final AtomicInteger NumberOfOpenFileHandles =
            new AtomicInteger(0);
     ...
     * 
     */
    public PersistentLikeHistory(
            final long uuid) {
        fileName = generateFileName(uuid);
        openFile();
    ...
    public static void initialize(final String storageDir)
        ...
        while (true) {
            String relPath = "";
            for (char counter : counters) {
                relPath += availableFolderNames[counter] + "/";
            }
    ...
    private void updateMemoryMappedBuffer(final long minimumBufferSize) {
        try {
            ...
            file =
                    RAFile.getChannel().map(MapMode.READ_WRITE, 0,
                            fileBufferSize);
        } catch (IOException e) {
            e.printStackTrace();
            System.exit(1);
    ...
    Like[] getLikesWithinTimePeriod(final int startTS, final int stopTS) {
        ...
        Like[] result = new Like[totalLines - currentLine];
        ...
        for (long line = currentLine; line < totalLines; line++) {
            ...
            result[resultPointer--] =
                    new Like(uuid, currentTs, Vote.getByFlag(flag));
        }
